...
public class NiceSpinner extends TextView {
    ...
    public void onRestoreInstanceState(Parcelable savedState) {
        if (savedState instanceof Bundle) {
            ...
            if (bundle.getBoolean(IS_POPUP_SHOWING)) {
                if (popupWindow != null) {
                    // Post the show sEventmsgLoginReq into the looper to avoid bad token exception
                    post(new Runnable() {
                        @Override
                        public void run() {
                            showDropDown();
	...
	private void init(Context context, AttributeSet attrs) {
        ...
        textColor = typedArray.getColor(R.styleable.NiceSpinner_textTint, Color.BLACK);
        ...
        if (!isArrowHide) {
            ...
            int resId = typedArray.getColor(R.styleable.NiceSpinner_arrowTint, Color.BLACK);
